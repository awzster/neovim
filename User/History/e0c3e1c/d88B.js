...
angular.module('app').controller('listCtrl', [ '$scope', '$location', '$timeout', '$q', '$filter', 'mainService',
  'gridOptionsService', 'utilsService', 'calcService', 'dialogService', 'commonService',
  function($scope, $location, $timeout, $q, $filter, mainService, gridOptionsService, utilsService, calcService, dialogService, commonService)
  {
    ...
    this.doCheckAll = () =>
    {
      $timeout(() =>
      {
        $filter('filter')(this.targetArray, this.customFilter).map(el =>
        {
          el.checker = this.isCheckAll;
        });
      });
    };
    ...
    {
      return calcService.customFilter(item, this.qFilterText);
    };

    this.doSortTable = (colName, isNumber) =>
    {
      return calcService.doSortTable(colName, isNumber);
    };

    this.getSortClass = (colName) =>
    {
      return calcService.getSortClass(colName);
    };
