...
angular.module('app').controller('treeCtrl', [ '$scope', '$location', '$stateParams', '$timeout', '$q', '$filter', 'mainService',
  ...
  function($scope, $location, $stateParams, $timeout, $q, $filter, mainService, gridOptionsTreeService, utilsService, calcService, commonService, NotifierService)
  {
    ...
    this.search = () =>
    {
      ...
      {
        this.idMap = new Map();
        data.forEach(item => this.idMap.set(item.id, item));
    ...
    this.moveDlgOpen = (id) =>
    {
      ...
      return mainService.httpGet({ dtType: 'nodeList', tree_id: this.tree_id }).then(data => 
      {
        this.nodeList = data;
        this.moveDlgOpen = true;
      });
    };
    ...
    this.moveSubmit = () =>
    {
      ...
      const params =  { dtType: 'moveSubmit', target: this.targetNode?.id, source: this.node.id };
      return mainService.httpPost(params).then(() => 
      {
    ...
    this.loadFilter().then(() =>
    {
      try
      {
        $q.all([ this.loadTree(), this.search() ]);
      }
      catch (e)
      {
