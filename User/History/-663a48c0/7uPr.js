...
angular.module('app').service('gridOptionsService', function($http, $q, $timeout, $filter, calcService, rendererService)
{
  ...
  this.init = () =>
  {
    const actionCellRenderer = ({ data }) =>
    {
      ...
      res += `
          <i class="pointer fa ${faElem}" ng-click="vm.changeStatus('${data.id}', 'TREE_FILTER', '${data.status}')"
        title="{{vm.getTitle('${data.status}')}}"></i>
          <span class="l-5"></span>
          <i class="pointer fa fa-pencil-alt text-primary" ng-click="vm.filterDlgOpen('${data.id}')" title="Edit Tree"></i>
          `
      ;
      res += '</span>';
      return res;
    ...
    const nameCellRenderer = ({ data, value }) =>
    {
      return data && !data.isTotal ?
        `<a href="javascript:void(0)" ng-click="vm.openFilter('${data.id}')">${value}</a>` : value;
    };
